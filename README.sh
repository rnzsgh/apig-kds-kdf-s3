...
A sample [create script](blob/master/create.sh) is included to launch the stack. The stack supports TLS via a certificate generated in [AWS Certificate Manager](https://aws.amazon.com/certificate-manager/). If your DNS is hosted [Amazon Route 53](https://aws.amazon.com/route53/) you can optionally register a DNS record for the API Gateway endpoint.
